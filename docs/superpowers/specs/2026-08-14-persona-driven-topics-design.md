...
**已知已验证无害的技术债——无档案时 wire 层 system prompt 比八期前多一个未说明的 `pillarHit` 属性：**`RADAR_READ.buildSystemPrompt(personaSection)`（`src/lib/llm/prompts/radar-read.ts`）在 `personaSection` 缺省/空串时返回值与八期前**字符级一致**（函数上方注释与单测均已保证）——但这只是 prompt *文本* 的一半。真正发给模型的 wire payload 由 `VisionLLM.callViaJsonMode`（`src/lib/llm/vision.ts:113-134`）拼装：`finalSystem = opts.systemPrompt + schemaHint`，其中 `schemaHint` 是 `zodToJsonSchema(opts.responseSchema)` 序列化后的 JSON Schema。`RadarReadResponseSchema`（同文件 `radar-read.ts:12-22`）**无条件**含 `pillarHit: z.string().max(10).nullable().optional().default(null)` 字段——它不感知 `personaSection` 是否为空，因此无档案场景下模型看到的 JSON Schema 里仍然多了一个八期前不存在、且没有任何自然语言说明其含义的 `pillarHit` 属性（`buildSystemPrompt` 只在 `hasPersona` 为真时才会在 `personaBlock` 里解释这个字段该怎么填）。也就是说："`buildSystemPrompt` 返回值字符级一致" 这条兼容性保证只覆盖了 prompt 正文，未覆盖 schema hint 这一段——两者拼接后的最终 wire payload 是有偏差的。

无害性已双重验证：(a) 静态——`validatePillarHit`（`src/lib/persona/profile.ts:81-85`）对无档案场景传入的 `pillars` 恒为 `[]`，`pillars.find(...)` 恒返回 `undefined`，函数恒返回 `null`，与模型在无字段说明时会输出什么值（`null`/瞎猜的字符串）无关——下游 `heatFactors.pillarHit` 恒为 `null`，`personaAdjust` 恒为 `0`，评分与排序不受影响；(b) 实测——上方「④无档案回退」E2E 复现 7/7 条命中结果 `pillarHit: null`、`personaAdjust: 0` 全部干净，无一条被模型瞎填的值污染。综上记为**已知已验证无害**的技术债，不阻塞本期收尾。可选加固（留后期，不在本期范围）：给 `pillarHit` 加 `.describe(...)` 让 schema 自解释，或按「有档案/无档案」拆两份 `responseSchema`（有档案版本才含 `pillarHit`）从根上消除该字段在无档案场景下的存在。

...
同一根因（`scriptDraftId` 不落 `CockpitContent`）还有另一层客户端影响，六期终审补记：抽屉内两级改稿 UI（素材简报折叠区、分块「换一版」、hook 候选切换、整体指令——即 `content-drawer.tsx` 的 `ScriptSectionsPanel` 及其驱动 state）全部挂在纯前端派生 state（`scriptDraftId`/`sections`/`research`/`hooks` 等，T7 注释已写明"不进 `WorkspaceState`"），不落库。抽屉关闭、或切换到另一个 item（触发 `resetScriptGenerationState`）后，这套 state 随之清空，不可恢复；已经 `mergeScript` 落盘的六字段骨架文本（`item.script.headline/hook/conclusion/body/example/ending`）不受影响、仍在，但要再用分块改稿/hook 候选功能，只能重新点「用 AI 写脚本」整篇重生成，拿到新的 `scriptDraftId`。`GET /api/v1/scripts/[id]` 路由本身已存在、能按 id 完整取回 `ScriptDraft`（含 sections/research/hooks），中期修法可与 (c) 项一并解决：要么生成时把 `scriptDraftId` 关联记录到 `CockpitContent`，要么抽屉打开时按记录的 id 懒加载拉回草稿、重建这套 state——两条路都留待下期，本期未做。

...
// `embedded`: 三期 T4 起 ReviewView 只从 AnalyticsView（目标/复盘 tab 容器）挂载——容器
// 自己已有一份 `.page-heading`，true 时隐藏本组件的整段标题，避免同屏两份「页面标题」堆叠。
export function ReviewView({ state, pageTitle, updateTitle, open, setState, embedded = false }: { state: WorkspaceState; pageTitle: string; updateTitle: (value: string) => void; open: (id: string) => void; setState: React.Dispatch<React.SetStateAction<WorkspaceState>>; embedded?: boolean }) {
  ...
  return <section className="page review-page">{!embedded ? <div className="page-heading"><span className="eyebrow">REVIEW LAB</span><EditablePageTitle value={pageTitle} fallback={DEFAULT_PAGE_TITLES.review} onChange={updateTitle} /><p>发布后自动进入待复盘；只有点击“保存复盘”，才会计入已复盘。</p></div> : null}

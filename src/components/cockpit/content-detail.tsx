...
import { SixActGuidePanel } from "@/components/cockpit/six-act-guide-panel";
...
export function ContentDetailView({ item, initialTab, stageEvents, stageColors, contentTypes, scriptDraftIdOverride, onScriptDraftLinked, update, mergeScript, changeStage, setStageStatus, schedule, unschedule, remove, markPublished, unmarkPublished, saveReview, ruleDeposited, addRule, notify }: { item: ContentItem; initialTab: ContentDrawerTab; stageEvents: StageEvent[]; stageColors: WorkspaceState["stageColors"]; contentTypes: string[]; /** T2 六期修复轮 1: Cockpit.tsx 维护的同会话客户端覆盖表, 优先于 item.scriptDraftId (服务端字段, 只有下次 workspace GET 才会刷新) */ scriptDraftIdOverride?: string; onScriptDraftLinked?: (contentId: string, scriptDraftId: string) => void; update: (patch: Partial<ContentItem>) => void; mergeScript: (id: string, partial: Partial<ContentItem["script"]>) => void; changeStage: (stage: ContentStage) => void; setStageStatus: (stage: WorkStage, completed: boolean) => void; schedule: (stage: WorkStage, plannedDate: string) => void; unschedule: (stage: WorkStage) => void; remove: () => void; markPublished: () => void; unmarkPublished: () => void; saveReview: () => void; ruleDeposited: boolean; addRule: (text: string) => void; notify: (message: string) => void }) {
  ...
  // 十三期收尾修复: 默认值改 90 且类型加 90 —— 此前固定 45 且下拉框只有
  // 30/45/60, 生成路由永远收到显式 durationSec=45, 后端「六幕默认 90 秒」的
  // 改动在抽屉入口从未真正生效过。
  const [durationSec, setDurationSec] = useState<30 | 45 | 60 | 90>(90);
  ...
  // 任何输入)。T6 六期 + 十三期收尾修复: parseDraftOutput 现在能解析出三种互斥
  // 形状——六幕稿 (`acts`+`four_dims` 都合法)、douyin sections (`sections` 非空)、
  // xiaohongshu (`intro`+`body` 都非空), 按 `parsed.acts`/`parsed.sections` 是否
  // 存在分流到对应的恢复分支, 三条分支都会写 scriptDraftId 触发面板渲染。
    ...
    (async () => {
      try {
        ...
        if (parsed.acts && parsed.four_dims) {
          // 十三期收尾修复: 六幕稿懒加载重开 —— 此前 parseDraftOutput 不识别
          // acts 形态, 关抽屉重开同一条已生成六幕稿的内容会导致 SixActPanel
          // 整体消失(不崩溃, 但改稿功能不可用), 与 sections/xhs 两条分支同一
          // 恢复模式补上这一支。
          setScriptDraftId(draftId);
          scriptDraftIdRef.current = draftId;
          setSixActScript({ acts: parsed.acts, four_dims: parsed.four_dims });
          if (parsed.lintIssues) setLintIssues(parsed.lintIssues);
  ...
  return <div className="page content-detail-page">
    ...
    {activeTab === "editing" ? <div className="drawer-section"><div className="stage-detail-strip"><span>剪辑阶段</span><Badge tone="editing" color={stageColors.editing}>剪辑</Badge><small>完成后进入发布</small></div><StageScheduleField item={item} stage="editing" stageEvents={stageEvents} schedule={schedule} unschedule={unschedule} />{sixActScript ? <SixActGuidePanel acts={sixActScript.acts} progress={item.editingActProgress} mode="editing" onToggle={(actKey, done) => update({ editingActProgress: { ...item.editingActProgress, [actKey]: done } })} /> : <><label className="field full"><span>剪辑备注</span><textarea className="large" value={item.editingNotes} onChange={(e) => update({ editingNotes: e.target.value })} placeholder="记录结构删改、字幕、包装、素材替换和导出要求…" /></label><div className="checklist"><strong>剪辑完成清单</strong>{["开头 5 秒直接进入场景", "案例或演示重点清楚", "字幕清楚可读", "封面与标题已确认", `${item.tier}档制作投入已控制`].map((text) => <label key={text}><input type="checkbox" />{text}</label>)}</div></>}</div> : null}

...
**energy (情绪基调, 2-200 字)**
他**本人**的表达能量 (如"自信、有感染力"/"冷静克制"/"自嘲式幽默")。这决定写稿时的
表达强度, 不是语言技巧 (那归风格档案)。两条硬约束:
- **只能从用户自己的话里提取, 不许发明**。第⑤问问的是"希望观众看完什么感觉", 那是
  *观众的* 感受, 不等于 *他的* 表达风格——观众"被感染到"通常意味着他本人是有感染力、
  自信的, 不要反推成"冷静克制"这类相反的特质。
- **不得与 notIdentity 冲突**。用户说自己不是技术极客, 就不能写"带点理工男的实在"这
  类暗示技术身份的描述。写之前先回头看一眼 notIdentity。
如果用户的回答里确实没有任何关于自己表达方式的线索, 宁可写得笼统 (如"真诚、平实"),
也不要编一个具体但没依据的人设。

...
export function isStageInFlow(platform: string, stage: ContentStage): boolean {
  ...
  return stageFlowFor(platform).includes(stage);
}

export function nextStageFor(platform: string, stage: ContentStage): ContentStage | null {
  const flow = stageFlowFor(platform);
  if (isStageInFlow(platform, stage)) {
  ...
  for (let i = pos + 1; i < CONTENT_STAGES.length; i += 1) {
    ...
    if (isStageInFlow(platform, candidate)) return candidate;
  }
  return null;
}

export function schedulableStagesFor(platform: string): WorkStage[] {
  const flow = stageFlowFor(platform);

...
// goals/review 是旧 NavigationItemId 的残留 —— 已从侧栏拿掉 (T2), 但内部跳转和历史
// `?view=` 链接仍需要这两个分支可达, 因此暂留在联合里。T4 移除: 把这两个视图重新挂到
// 新 IA 后一并清理。schedule 已并入 momentum 的档期 tab (T3), 不再是独立 NavView ——
...
/**
 * 解析 `?view=` 初始视图。合法固定项 / 平台项 / legacy (goals|review) 放行,
 * `settings` 单独放行, 其余（含缺省、非法值、旧的 `schedule`）一律回退 `"momentum"`。
 * `schedule` 在 T3 后不再是独立 NavView —— 旧 `?view=schedule` 链接落回 momentum(今日),
 * T6 再做到档期 tab 的精确映射。
 */
export function resolveInitialView(searchParams: URLSearchParams): NavView {
  ...
  if (FIXED_VIEW_IDS.includes(requested) || LEGACY_VIEW_IDS.includes(requested)) {
